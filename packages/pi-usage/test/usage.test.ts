...
test("View all renders unresolved target providers without opening a nested picker", async (t) => {
  ...
  globalThis.fetch = async (input) => {
    ...
    if (url.includes("/v1/accounts?")) {
      return new Response(JSON.stringify({ accounts: [{ name: "accounts/acme" }, { name: "accounts/beta" }] }), {
        status: 200,
      });
    }
    return usageFetch(input);
...
test("session shutdown aborts usage action and provider selectors", async (t) => {
  ...
  const { ctx } = createMockContext({
    ...
    mode: "rpc",
    model: openRouterModel,
    select: async (title: string, _options: string[], opts?: { signal?: AbortSignal }): Promise<string | undefined> => {
  ...
  settings.runtime.reload = async (signal) => {
    ...
    if (reloads === 1) {
      ...
      await new Promise<void>((_resolve, reject) => {
        signal?.addEventListener("abort", () => reject(new DOMException("Settings reload aborted", "AbortError")), {
          once: true,
        });
      });
    }
  ...
  const { ctx } = createMockContext({
    ...
    custom: async (factory: unknown) =>
      new Promise<boolean>((resolve) => {
        ...
        )({ requestRender() {} }, { bold: (text) => text, fg: (_color, text) => text }, conflictingKeybindings, done);
        component.handleInput("\u0003");
...
test("settings cancellation aborts a stalled save and closes without awaiting it", async (t) => {
  ...
  for (const cancellation of ["ctrl+c", "configured cancel", "parent abort"] as const) {
    ...
    let markReady: (component: { handleInput(data: string): void; render(width: number): string[] }) => void = () =>
      undefined;
    ...
    const { ctx } = createMockContext({
      ...
      custom: async (factory: unknown) =>
        new Promise<boolean>((resolve) => {
          ...
          )({ requestRender() {} }, { bold: (text) => text, fg: (_color, text) => text }, remappedKeybindings, done);
          markReady(component);
        }),
    });
  ...
  const { ctx } = createMockContext({
    ...
    custom: async (factory: unknown) =>
      new Promise<boolean>((resolve) => {
        ...
        )({ requestRender() {} }, { bold: (text) => text, fg: (_color, text) => text }, { matches: () => false }, done);
        component.handleInput("\u001b[B");
        component.handleInput("\r");
      }),
  });
