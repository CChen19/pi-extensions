...
Startup checks never open a dialog. With a sync baseline and no content-list mismatch, status uses **⇡** for local changes to push and **⇣** for remote changes to pull. Review conditions use **⇕** with a persistent widget above the editor: both sides changing, first sync with an existing remote snapshot, a differing content list (including order-only differences), or a missing remote snapshot despite an existing baseline. Both sides changing is a reason to review, not a confirmed file conflict.
